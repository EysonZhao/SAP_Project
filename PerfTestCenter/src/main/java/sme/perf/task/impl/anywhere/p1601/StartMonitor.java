...
public class StartMonitor extends Task {
	...
	public Result execute() throws ParameterMissingException {
		...
		logger.info("Start to Start Monitor.");
		try {
			for (Host host : allMachine) {
				String ResMonitorCmd = "cd " + mainfolder + " && ./OSInfoSummary.sh && ./ResMonitor*.sh";
				if (mariadbServerList!=null && mariadbServerList.contains(host)) {
					ResMonitorCmd = "cd " + mainfolder
							+ " && ./OSInfoSummary.sh && ./ResMonitor*.sh mysql";
				}
				if (hanaServerList.contains(host)) {
					ResMonitorCmd = "cd " + mainfolder + " && ./OSInfoSummary.sh && ./ResMonitor*.sh "
							+ hanaUser;
				}
				if (etcdServerList.contains(host)|| shareFolderServerList.contains(host)) {
					ResMonitorCmd = "cd " + mainfolder
							+ " && ./OSInfoSummary.sh && ./ResMonitor*.sh root";
				}
				...
				rrs.execute(ResMonitorCmd,2);
			}
			logger.info("Finish to Start Monitor.");
	...
	public TaskParameterMap getParameters() {
		if (null == this.parameters) {
			this.parameters = StartMonitor.getParameterTemplate();
		}
	...
	public TaskParameterMap getHosts() {
		if (null == this.hostParameters) {
			this.hostParameters = StartMonitor.getHostsTemplate();
		}
	...
	public static String getDescriptionTemplate() {
		return new StartMonitor().getDescription();
	}
}
