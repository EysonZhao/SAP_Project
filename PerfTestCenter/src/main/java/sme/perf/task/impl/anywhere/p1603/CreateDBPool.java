		...
		try {
			...
			for (int i = startSchemaNumber; i < startSchemaNumber + tenantNumber; i++) {
				...
				String createDBPoolCmd = "mysql -h "+csmServer.getIP()+" -uroot -p"
						...
						+ mariaDBPasswd
						+ " -e \"INSERT INTO CSM.DBSCHEMA (id,name,type,db_cluster_id,status,istrustedconnection,lifecyclestatus,substatus,version,create_time,update_time,db_schema_type) values ("
						+ indexID
						+ ",'PERFDB"
						+ i
