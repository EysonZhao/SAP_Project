...
public class CleanEnvironment extends Task {
	...
	public Result execute() throws ParameterMissingException {
		...
		try {
			RunRemoteSSH rrs;
			String getSchemaNameCMD = "mysql -uroot -p"+mariaDBPasswd+" -e \"select SCHEMA_NAME from information_schema.SCHEMATA where SCHEMA_NAME like '%ESHOP%'\"";
			rrs=new RunRemoteSSH(csmServer.getIP(), csmServer.getUserName(), csmServer.getUserPassword());
			rrs.setLogger(logger);
			String schemaNameList=rrs.executeWithReturnString(getSchemaNameCMD);
			String[] schemaList=schemaNameList.split("\n");
			//TODO MODIFY THE LIST if necessary
			List<Host> eshopDBRelatedHostList=new ArrayList<Host>();
			eshopDBRelatedHostList.addAll(csmServerList);
			//Fix MariaDB Empty
			if (mariadbServerList!=null){
				eshopDBRelatedHostList.addAll(mariadbServerList);
			}
			for (Host host:eshopDBRelatedHostList){
				logger.info("Current machine:"+host.getIP());
				rrs=new RunRemoteSSH(host.getIP(), host.getUserName(), host.getUserPassword());
				rrs.setLogger(logger);
				schemaNameList=rrs.executeWithReturnString(getSchemaNameCMD);
				schemaList=schemaNameList.split("\n");
				for(int i=1;i<schemaList.length-1;i++){
					LogHelper.info("schema ="+schemaList[i]);
					String dropEshopDBCMD = "mysql -uroot -p"+mariaDBPasswd+" -e \"DROP DATABASE "+schemaList[i]+"\"";
					rrs=new RunRemoteSSH(host.getIP(), host.getUserName(), host.getUserPassword());
					rrs.setLogger(logger);
					rrs.execute(dropEshopDBCMD);
				}
			}	
			
			String dropCSMDBCmd = "mysql -uroot -p"+mariaDBPasswd+" -e \"DROP DATABASE CSM\"";
			rrs=new RunRemoteSSH(csmServer.getIP(), csmServer.getUserName(), csmServer.getUserPassword());
			rrs.setLogger(logger);
			rrs.execute(dropCSMDBCmd,true);
	...
	public TaskParameterMap getParameters() {
		if (null == this.parameters) {
			this.parameters = CleanEnvironment.getParameterTemplate();
		}
	...
	public TaskParameterMap getHosts() {
		if (null == this.hostParameters) {
			this.hostParameters = CleanEnvironment.getHostsTemplate();
		}
