...
//			String mariadbImage=rrs.executeWithReturnString("docker ps | grep mariadb | awk '{print $1}'");
			for (int i = startSchemaNumber; i < startSchemaNumber + tenantNumber; i++) {
				int indexID = i + 15000;
				String createDBPoolCmd = "mysql -h "+csmServer.getIP()+" -uroot -p"
						+ mariaDBPasswd
						+ " -e \"INSERT INTO CSM.COMPANYDB values ('PERFDB"
						+ i
						+ "', 'CN', (select GUID from CSM.DKEYS where id = 1), (select id from CSM.SERVICE_UNIT where status = 'ONLINE'),"
						+ indexID
						+ ")\"\n"
						+"mysql -h "+csmServer.getIP()+" -uroot -p"
						+ mariaDBPasswd
						+ " -e \"INSERT INTO CSM.DBSCHEMA (id,name,type,db_cluster_id,status,istrustedconnection,lifecyclestatus,substatus,version,create_time,update_time,db_schema_type,package_component_id) values ("
						+ indexID
						+ ",'PERFDB"
						+ i
						+ "','POOL', (select id from CSM.BASE_CLUSTER where type = 'HANADB'),'ONLINE',0,'RUNNING','Normal','"
						+ version + "','2014-09-05','2014-09-05','COMPANY',(select id from CSM.PACKAGE_COMPONENT where NAME='OCCDB'))\"";
