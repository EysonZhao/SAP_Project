...
public class OCCSizingSingleCase extends Task {
	...
	public Result execute() throws ParameterMissingException {
		...
		try {
			...
			String getSLDurlCMD = "mysql -h "+csmServer.getIP()+" -uroot -p"+mariaDBPasswd+" -e \"select T0.URL FROM CSM.SERVICE_CLUSTER T0, CSM.BASE_CLUSTER T1 WHERE T0.BASE_CLUSTER_ID=T1.ID AND T1.\\`TYPE\\`='IDP'\"";
			String getSPurlCMD = "mysql -h "+csmServer.getIP()+" -uroot -p"+mariaDBPasswd+" -e \"select T0.URL FROM CSM.SERVICE_CLUSTER T0, CSM.BASE_CLUSTER T1 WHERE T0.BASE_CLUSTER_ID=T1.ID AND T1.\\`TYPE\\`='SOLUTIONPORTAL'\"";
