...
public class ShellScriptCopy extends Task {
		...
		try {
			...
			for (Host host:allMachine){
				...
				if (csmServerList.contains(host)){
					//Orginal
					//String prepareJmeterFolderCmd="if [ ! -d /opt/apache-jmeter-2.9/ ] ; then mkdir /opt/apache-jmeter-2.9/ && chmod 777 /opt/apache-jmeter-2.9/ ; fi && rm -rf /opt/apache-jmeter-2.9/* ";
					//RunRemoteSSH.execute(host.getIP(),host.getUserName(),host.getUserPassword(),prepareJmeterFolderCmd);
					...
					//RunRemoteSSH.execute(centralServer.getIP(),centralServer.getUserName(),centralServer.getUserPassword(),csmJmeterCmd);
	...
	public TaskParameterMap getParameters() {
		if (null == this.parameters) {
			this.parameters = ShellScriptCopy.getParameterTemplate();
		}
	...
	public TaskParameterMap getHosts() {
		if (null == this.hostParameters) {
			this.hostParameters = ShellScriptCopy.getHostsTemplate();
		}
