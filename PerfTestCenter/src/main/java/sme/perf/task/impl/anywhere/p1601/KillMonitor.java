...
public class KillMonitor extends Task {
	...
	public Result execute() throws ParameterMissingException {
		...
		logger.info("Start to Kill Monitor");
		try {		
			String killMonitorCmd = "cd " + mainfolder
					+ " && ./KillMonitor*.sh";
			for (Host host : allMachine) {
				...
				rrs.execute(killMonitorCmd, true);
			}
			
			logger.info("Finish to Kill Monitor.");
	...
	public TaskParameterMap getParameters() {
		if (null == this.parameters) {
			this.parameters = KillMonitor.getParameterTemplate();
		}
	...
	public TaskParameterMap getHosts() {
		if (null == this.hostParameters) {
			this.hostParameters = KillMonitor.getHostsTemplate();
		}
	...
	public static String getDescriptionTemplate() {
		return new KillMonitor().getDescription();
	}
