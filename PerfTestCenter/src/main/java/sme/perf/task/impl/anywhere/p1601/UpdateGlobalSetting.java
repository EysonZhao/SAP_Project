...
public class UpdateGlobalSetting extends Task {
	...
	public Result execute() throws ParameterMissingException {
		...
		try {
			...
			String updateSettingCommonCmd="mysql -uroot -p"+mariaDBPasswd+" -e \"UPDATE CSM.GLOBALSETTING SET VALUE=9999 WHERE \\`KEY\\` = 'MaxUsersPerTrialCustomer'\"";
	...
	public TaskParameterMap getParameters() {
		if (null == this.parameters) {
			this.parameters = UpdateGlobalSetting.getParameterTemplate();
		}
	...
	public TaskParameterMap getHosts() {
		if (null == this.hostParameters) {
			this.hostParameters = UpdateGlobalSetting.getHostsTemplate();
		}
